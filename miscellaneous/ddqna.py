...
class TD3Agent:
    ...
    def __init__(self, action_space, epsilon=1.0, gamma=0.95, epsilon_min=0.1, epsilon_decay=0.9999, lr=0.001, memory_len=5000, frames=1, hidden_dimension=128, device=None, tau=0.005, policy_noise=0.2, noise_clip=0.5, policy_freq=2):
    ...
    def replay(self, batch_size):
        ...
        action = torch.tensor(action).to(self.device).unsqueeze(1)  # Unsqueeze to match dimensions
        ...
        loss_q1.backward(retain_graph=True)  # Retain the graph for multiple backward passes
...
def compare_model_weights(model, checkpoint):
    ...
    for key in model_weights:
        if not torch.equal(model_weights[key], checkpoint_weights[key]):
...
if __name__ == "__main__":
    ...
    pacman = TestTraining('ALE/Pacman-v5', episodes=10, batch_size=1)
