...
class DQN(nn.Module):
    def __init__(self, in_channels, num_actions, h_dimension):
        ...
        self.layers_cnn = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(64, 128, kernel_size=(3, 3), stride=1),  # Increased filter size
            nn.ReLU(),
            nn.Conv2d(128, 128, kernel_size=(3, 3), stride=1),  # Added extra convolutional layer
            nn.ReLU(),
            nn.Flatten()
        )
        ...
        self.fc = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(h_dimension, h_dimension),  # Added an additional fully connected layer
            nn.ReLU(),
...
class TD3Agent:
    ...
    def __init__(self, action_space, epsilon=1.0, gamma=0.98, epsilon_min=0.1, epsilon_decay=0.995, lr=0.001, memory_len=50000, frames=1, hidden_dimension=512, device=None, tau=0.01, policy_noise=0.1, noise_clip=0.3, policy_freq=1):
...
if __name__ == "__main__":
    model_path = "mspacman_dqn_model_imp.pth"
