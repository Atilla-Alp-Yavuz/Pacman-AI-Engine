...
def optimize_hyperparameters():
    ...
    space = [
        Real(0.95, 0.98, name='gamma'),
        Real(0.9995, 0.9999, name='epsilon_decay'),
        Integer(50000, 100000, name='memory_len'),
        Integer(128, 256, name='hidden_dimension')
    ]

    @use_named_args(space)
    def objective(gamma, epsilon_decay, memory_len, hidden_dimension):
        ...
        avg_reward = np.mean(trainer.rewards[-100:])
        return -avg_reward
        
    result = gp_minimize(objective, space, n_calls=30, random_state=42)
    print("Best parameters found:")
    print("Gamma:", result.x[0])
    print("Epsilon decay:", result.x[1])
    print("Memory length:", result.x[2])
    print("Hidden dimension:", result.x[3])
